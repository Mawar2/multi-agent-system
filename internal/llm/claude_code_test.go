...
func TestClaudeCodeBackend_Execute_UnsupportedModel(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "gpt-4") {
		t.Errorf("Execute error = %q, want it to contain 'gpt-4'", err.Error())
	}
}

// TestClaudeCodeBackend_Execute_DefaultModel verifies that an empty model string uses the default.
// This test calls the real Claude CLI and is skipped unless the integration flag is set.
