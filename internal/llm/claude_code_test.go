...
// TestClaudeCodeBackend_Models verifies the available models include current IDs.
func TestClaudeCodeBackend_Models(t *testing.T) {
	...
	for _, required := range requiredModels {
		...
		if !found {
			t.Errorf("Models() missing expected model %q", required)
		}
	}
}
...
func TestClaudeCodeBackend_Execute_UnsupportedModel(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "unsupported model") {
		t.Errorf("Execute error = %q, want it to contain 'unsupported model'", err.Error())
	}

	if !strings.Contains(err.Error(), "gpt-4") {
		t.Errorf("Execute error = %q, want it to contain the model name 'gpt-4'", err.Error())
	}
}

// TestClaudeCodeBackend_Execute_DefaultModel verifies that an empty model skips to the real CLI.
