...
func TestClaudeCodeBackend_Models(t *testing.T) {
	...
	// Verify current models are present (at minimum)
	required := []string{
		"claude-sonnet-4-6",
		"claude-opus-4-8",
		"claude-haiku-4-5",
	...
	for _, expected := range required {
		if !modelSet[expected] {
			t.Errorf("Models() missing required model %q", expected)
		}
	}
...
func TestClaudeCodeBackend_Execute_UnsupportedModel(t *testing.T) {
	...
	if err == nil || !strings.Contains(err.Error(), "execute: unsupported model \"gpt-4\"") {
		t.Errorf("Execute error = %q, want unsupported model error", err)
	}
}

// TestClaudeCodeBackend_Execute_DefaultModel verifies default model selection does not error on empty model.
func TestClaudeCodeBackend_Execute_DefaultModel(t *testing.T) {
	t.Skip("Skipping integration test - calls real Claude CLI")

	backend := NewClaudeCodeBackend()
	ctx := context.Background()
	prompt := "test prompt"

	// Empty model should use default (claude-sonnet-4-6) without an unsupported model error.
