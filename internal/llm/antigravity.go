...
const defaultAntigravityBaseURL = "http://localhost:4000"

// LocalAntigravityBackend implements LLMBackend via the Antigravity bridge,
// which exposes an OpenAI-compatible /v1/chat/completions endpoint backed by
// local Gemini models. Set ANTIGRAVITY_BASE_URL to override the default URL
// (useful in tests and staging environments).
...
// ANTIGRAVITY_BASE_URL env var overrides the default base URL for testability.
...
// Execute sends a prompt to the Antigravity bridge (delegates to ExecuteInDir).
...
// workDir is not used: the bridge manages its own workspace context.
func (b *LocalAntigravityBackend) ExecuteInDir(ctx context.Context, prompt string, model string, _ string) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("execute: HTTP request failed: %w", err)
	}
	...
	if err := json.Unmarshal(body, &result); err != nil {
		return "", fmt.Errorf("execute: parse response: %w", err)
	}
