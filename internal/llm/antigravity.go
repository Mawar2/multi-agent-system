...
// bridge's OpenAI-compatible /v1/chat/completions endpoint.
//
...
// The workDir parameter is accepted for interface compatibility but not used,
// since Antigravity runs as a remote service.
func (b *LocalAntigravityBackend) ExecuteInDir(ctx context.Context, prompt string, model string, _ string) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("execute: failed to marshal request: %w", err)
	}

	url := b.baseURL + "/v1/chat/completions"
	req, err := http.NewRequestWithContext(ctx, http.MethodPost, url, bytes.NewReader(reqBytes))
	if err != nil {
		return "", fmt.Errorf("execute: failed to create request: %w", err)
	}
	...
	if err := json.NewDecoder(resp.Body).Decode(&chatResp); err != nil {
		return "", fmt.Errorf("execute: failed to decode response: %w", err)
	}
