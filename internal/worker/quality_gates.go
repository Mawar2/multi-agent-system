...
func (qg *QualityGates) Validate(ctx context.Context, ruleset *conventions.Ruleset, workspaceDir string) error {
	fmt.Printf("[QualityGates] Running pre-PR quality checks in %s\n", workspaceDir)

	// Check 1: Tests
	if err := qg.runTests(ctx, ruleset, workspaceDir); err != nil {
		return fmt.Errorf("quality gate failed - tests: %w", err)
	}

	// Check 2: Linter
	if err := qg.runLinter(ctx, ruleset, workspaceDir); err != nil {
		return fmt.Errorf("quality gate failed - linter: %w", err)
	}

	// Check 3: Formatter
	if err := qg.runFormatter(ctx, ruleset, workspaceDir); err != nil {
	...
	if ruleset.BuildCommand != "" {
		if err := qg.runBuild(ctx, ruleset, workspaceDir); err != nil {
...
func (qg *QualityGates) runTests(ctx context.Context, ruleset *conventions.Ruleset, workspaceDir string) error {
...
func (qg *QualityGates) runLinter(ctx context.Context, ruleset *conventions.Ruleset, workspaceDir string) error {
...
func (qg *QualityGates) runFormatter(ctx context.Context, ruleset *conventions.Ruleset, workspaceDir string) error {
...
func (qg *QualityGates) runBuild(ctx context.Context, ruleset *conventions.Ruleset, workspaceDir string) error {
...
func (qg *QualityGates) ValidateWithDetails(ctx context.Context, ruleset *conventions.Ruleset, workspaceDir string) []ValidationResult {
	results := make([]ValidationResult, 0)

	// Test check
	testErr := qg.runTests(ctx, ruleset, workspaceDir)
	...
	// Linter check
	lintErr := qg.runLinter(ctx, ruleset, workspaceDir)
	results = append(results, ValidationResult{
		CheckName: "linter",
		Passed:    lintErr == nil,
		Error:     lintErr,
	...
	// Formatter check
	fmtErr := qg.runFormatter(ctx, ruleset, workspaceDir)
	results = append(results, ValidationResult{
		CheckName: "formatter",
		Passed:    fmtErr == nil,
		Error:     fmtErr,
	...
	if ruleset.BuildCommand != "" {
		buildErr := qg.runBuild(ctx, ruleset, workspaceDir)
		results = append(results, ValidationResult{
			CheckName: "build",
			Passed:    buildErr == nil,
			Error:     buildErr,
