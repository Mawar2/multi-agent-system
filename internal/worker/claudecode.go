...
// workspaceProvider abstracts workspace lifecycle so tests can inject a mock
// without touching the real filesystem or running git commands.
...
// qualityValidator abstracts quality-gate execution so tests can inject a mock
...
type ClaudeCodeWorker struct {
	...
	qualityGate    qualityValidator    // Runs pre-PR quality checks
...
func (w *ClaudeCodeWorker) Execute(ctx context.Context, task *taskqueue.Task) (*Result, error) {
	...
	if err := w.qualityGate.Validate(ctx, workspaceDir, ruleset); err != nil {
