...
// workspaceProvider abstracts workspace preparation for hermetic testing.
...
// qualityValidator abstracts pre-PR quality validation for hermetic testing.
...
type ClaudeCodeWorker struct {
	...
	qualityGate    qualityValidator    // nil = create per-workspace (production); set = use as-is (tests)
