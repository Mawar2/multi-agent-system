...
// workspaceProvider is an unexported seam for workspace management.
// *WorkspaceManager satisfies this interface; tests inject a fake.
...
// qualityValidator is an unexported seam for pre-PR quality checks.
// *QualityGates satisfies this interface; tests inject a no-op.
...
type ClaudeCodeWorker struct {
	...
	gateFactory    func(dir string) qualityValidator // Creates quality gates for a workspace
...
func NewClaudeCodeWorker(
	...
	return &ClaudeCodeWorker{
		...
		gateFactory:   func(dir string) qualityValidator { return NewQualityGates(dir) },
	}
}
...
func (w *ClaudeCodeWorker) Execute(ctx context.Context, task *taskqueue.Task) (*Result, error) {
	...
	gates := w.gateFactory(workspaceDir)
