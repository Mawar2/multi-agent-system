...
```
Supervisor → Task Router → Task Queue → Worker Pool → GitHub PRs → Human Review
```
...
3. Task enqueued in JSON queue with tier assignment
...
8. CI runs (tests + linter + AI review)
9. Human reviews and merges
10. Supervisor marks complete, worker claims next task
